...
export class ChatReformaPage {
  ...
  constructor(
    ...
    this.socket = io.connect('http://minhareforma.herokuapp.com:52738/');
    this.socket.on("inserirMensagem", data => {
      console.log(data);
  ...
  enviaMensagem() {
    ...
    this.socket.emit('inserirMensagem', mensagem);
    this.getMessages().subscribe(msgs => {
      console.log(msgs);
